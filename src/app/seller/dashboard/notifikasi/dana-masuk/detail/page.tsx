...
const NotificationDetailPage = () => {
  ...
  const handleClick = () => {
    ...
    router.push("/seller/dashboard/setup-toko/pusat-bantuan");
  };

  const handleConfirm = () => {
    console.log("button di klik");
    router.push("/seller/dashboard/notifikasi");
  };
  return (
    <LayoutUtama>
      <div className="flex flex-col min-h-screen font-nunito">
        ...
        <div className="container mx-auto p-4 flex-grow">
          ...
          <div className="mt-4 text-center">
            <h1 className="font-bold text-[24px] text-black">Dana Masuk</h1>
            <p className="text-sm text-black mt-4">
              Selamat! Pesanan anda selesai dan mendapatkan Rp{" "}
              {new Intl.NumberFormat("id-ID").format(orderDetail.price)} dari
              pesanan dengan ID {orderDetail.id}
            </p>
          </div>
          ...
          <div
            className="text-right mt-4 text-[#0095FF] text-sm hover:underline cursor-pointer"
            onClick={handleClick}
          >
            Belum menerima dana? Butuh Bantuan
          </div>
        </div>
