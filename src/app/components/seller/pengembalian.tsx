...
export default function DiKembalikan() {
  const orders = [
    {
      ...
      product: {
        ...
        variant: 'XL, Putih',
        quantity: 1,
        orderDate: '2 Desember 2024',
        submissionDate: '19 Desember 2024',
      },
    },
    {
      orderId: '1270511325',
    ...
    },
    {
      ...
      product: {
        ...
        quantity: 1,
        orderDate: '12 Desember 2024',
        submissionDate: '22 Desember 2024',
      },
    },
  ]

  const [expiredOrders, setExpiredOrders] = useState<boolean[]>([])
  ...
  //     const updatedExpiredOrders = orders.map((order) => {
  //       const orderDate = parse(order.product.orderDate, format, new Date(), {
  //         locale: id,
  //       })
  //       if (isNaN(orderDate.getTime())) {
  //         console.error('Invalid order date:', order.product.orderDate)
  //       }
        
  //       const expirationDate = new Date(orderDate) // Salin tanggal
  //       expirationDate.setDate(expirationDate.getDate() + 7)
  //        // 7 hari dari orderDate

  //       const isExpired = currentDate > expirationDate
  //       return isExpired
  //     })

  //     setExpiredOrders(updatedExpiredOrders)
  //   }
  ...
  useEffect(() => {
    const checkExpiration = () => {
      ...
      const updatedExpiredOrders = orders.map((order) => {
        ...
        expirationDate.setDate(orderDate.getDate() + 7)
      ...
      {orders.map((order, idx) => (
        ...
        >
          ...
          <div className="flex justify-between text-xs text-gray-600 font-nunito">
            <div>
              <b>Tanggal Order</b>
              <div>{order.product.orderDate}</div>
            </div>
            ...
            <div>
              ...
              <div className="text-right">{order.product.submissionDate}</div>
            </div>
          </div>

          {/* Waktu Pengembalian Kadaluarsa */}
          {expiredOrders[idx] ? (
            <div className="flex flex-col w-full space-y-4">
              <span className="flex text-xs justify-start gap-2 font-semibold items-center py-3 px-2 rounded-lg bg-[#FCDAD9] text-red-500">
                <svg
                  width="14"
                  height="15"
                  viewBox="0 0 14 15"
                ...
                >
                  <path
                    d="M6.99935 13.3334C3.77769 13.3334 1.16602 10.7217 1.16602 7.50008C1.16602 4.27842 3.77769 1.66675 6.99935 1.66675C10.221 1.66675 12.8327 4.27842 12.8327 7.50008C12.8327 10.7217 10.221 13.3334 6.99935 13.3334ZM6.41602 9.25008V10.4167H7.58268V9.25008H6.41602ZM6.41602 4.58341V8.08341H7.58268V4.58341H6.41602Z"
                    fill="#EE443F"
                  />
                </svg>
                Waktu Pengembalian Kadaluarsa
              ...
              <div className="flex gap-3">
                ...
                <button
                  className="w-1/2 py-2 rounded-lg bg-gray-400 text-white text-sm font-bold cursor-not-allowed"
                  disabled
                >
                  Proses Pengembalian
                </button>
            ...
            <div className="flex gap-3">
              <button className="w-1/2 py-2 rounded-lg bg-[#51d7b1] text-white text-sm font-bold">
              {/*Nanti masuk ke bagian notifikasi buyer, jika pengajuan ditolak*/}
                Tolak Pengajuan
              </button>
              <button className="w-1/2 py-2 rounded-lg bg-[#51d7b1] text-white text-sm font-bold">
                <Link href={`/seller/dashboard/notifikasi/dikomplain/rincian`}>
                  Proses Pengembalian
                </Link>
              </button>
            </div>
